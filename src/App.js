...
function App() {
  ...
  const changePosition = useCallback((items, setItems, item, number) => {
    if (winner) {
      return
    }
    if (!item || !item.isNext) {
  ...
  const playerOneMappedKeys = playerOneItems.filter(i => i.isNext).reduce((acc, i) => {
  ...
  const playerTwoMappedKeys = playerTwoItems.filter(i => i.isNext).reduce((acc, i) => {
  ...
  useEffect(() => {
    const upHandler = e => {
      ...
      changePosition(playerOneItems, setPlayerOneItems, playerOneMappedKeys[keyCode], 1)
      changePosition(playerTwoItems, setPlayerTwoItems, playerTwoMappedKeys[keyCode], 2)
    }
  ...
  }, [changePosition, playerOneItems, playerOneMappedKeys, playerTwoItems, playerTwoMappedKeys]);
            ...
            {playerOneItems.map(i => {
              ...
              return (
                <S.Item
                  ...
                  onClick={() => changePosition(playerOneItems, setPlayerOneItems, i)}
                  color={i.color}
            ...
            {playerTwoItems.map(i => {
              ...
              return (
                <S.Item
                  key={i.id}
                  style={{ left: `${i.x * 60}px`, top: `${i.y * 60}px` }}
                  onClick={() => changePosition(playerTwoItems, setPlayerTwoItems, i)}
                  color={i.color}
                  anime={i.current && i.position}
                />
              )
